...
| 2026-04-21 | ポーリングレートを 125Hz → 250Hz（デフォルト）に変更。センサー検出遅延を最大 8ms → 4ms に半減。加速カーブの `min-factor` を 700 → 1000（等倍）に変更。250Hz 化で動き終わりの低速サンプルが増えるため 0.7x 縮小が慣性感として知覚される問題を解消 |
