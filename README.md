...
| 2026-04-26 | 〈Drag Lock〉— C 実装の stateful behavior `zmk,behavior-drag-lock` を新設。`src/behaviors/behavior_drag_lock.c` で MB1 ホールド状態を内部フラグで保持し、ワンキー（L4 MOUSE 右親指最左、旧 `&drag_on` 位置）でトグル動作。リポジトリを ZMK in-tree module 化（`zephyr/module.yml` 拡張、`CMakeLists.txt` / `Kconfig` / `dts/bindings/behaviors/` を新設）|
